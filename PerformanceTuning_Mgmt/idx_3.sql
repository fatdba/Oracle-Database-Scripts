...
select a.OWNER ,SEGMENT_NAME,b.uniqueness,b.partitioned,b.index_type,b.status,TABLESPACE_NAME,sum(BYTES/1024/1024) tot_size_mb,c.column_name ,c.column_position col_pos,b.last_analyzed,decode(VISIBILITY,'VISIBLE','V','INVISIBLE','I') v
  from dba_segments a, (select owner,index_name,index_type,status,last_analyzed,uniqueness,partitioned,VISIBILITY from dba_indexes where table_name=( select table_name from dba_indexes where index_name='&&idx_name') and table_owner='BOOKER') b,dba_ind_columns c 
 ...
 select table_name from dba_indexes where index_name='&&idx_name' ;
